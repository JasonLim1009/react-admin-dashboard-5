...
$key[0] = "usg_itm_location";
$key[1] = "usg_itm_list";
$key[1] = "usg_itm_change";
...
$sql= "	SELECT 
		...
		WHERE 	usg_itm.site_cd = '".$site_cd."'
		AND 	usg_itm.RowID = '".$RowID."'";

	$stmt_usg_itm = sqlsrv_query( $conn, $sql);

	if( !$stmt_usg_itm ) {
		 $error_message = "Error selecting table (usg_itm)";
	...
	do {
        while ($row = sqlsrv_fetch_array($stmt_usg_itm, SQLSRV_FETCH_ASSOC)) {
    ...
    } while (sqlsrv_next_result($stmt_usg_itm));
