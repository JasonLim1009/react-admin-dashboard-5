...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    do {
        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
			...
			$row_result["col69"] = validate_date($row[$key[68]]);
			$row_result["col70"] = validate_date($row[$key[69]]);
			
			$row_result["col71"] = validate_date($row[$key[70]]);
            $row_result["col72"] = validate_date($row[$key[71]]);
            $row_result["col73"] = validate_date($row[$key[72]]);
